...
const otpVerificationController = async (req, res, next) => {
    ...
    try {
        ...
        isOtpValid
            ? (0, userProfileModel_1.userProfileModel)().findOneAndUpdate({ email: (_c = req.body) === null || _c === void 0 ? void 0 : _c.email }, { status: enum_1.registrationStatus.verified }, async (err, result) => {
                var _a;
                if (!err) {
...
const updateUserLoginCredStatus = (email) => {
    return new Promise((resolve, reject) => {
        try {
            (0, loginCredModel_1.loginCredModel)().findOneAndUpdate({ email: email }, { status: enum_1.registrationStatus.verified }, (err, result) => {
                if (!err) {
                    resolve(true);
                }
                else {
