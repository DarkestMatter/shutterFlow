...
const otpVerificationController = async (req, res, next) => {
    ...
    try {
        ...
        isOtpValid
            ? (0, userProfileModel_1.userProfileModel)().findOneAndUpdate({ email: (_c = req.body) === null || _c === void 0 ? void 0 : _c.email }, { status: "Verified" /* statusEnum.verified */ }, async (err, result) => {
                var _a;
                if (!err) {
        ...
        (0, responderController_1.responderController)({ result: {}, statusCode: 200, errorMsg: "Error occurred while validating OTP" /* errorMsg.errorAtOtp */ }, res);
    }
};
...
const updateUserLoginCredStatus = (email) => {
    return new Promise((resolve, reject) => {
        try {
            (0, loginCredModel_1.loginCredModel)().findOneAndUpdate({ email: email }, { status: "Verified" /* statusEnum.verified */ }, (err, result) => {
                if (!err) {
                    resolve(true);
                }
                else {
